...
visit_table = pd.read_csv(
    "/data/visit_occurrence.csv", usecols=["person_id", "visit_concept_id"]
)
...
def filterByPcrDate(table_name, column_name, filter_date):
    return table_name[table_name[column_name] >= str(filter_date)].sort_values(
        by=column_name
    )


measurements_table = pd.read_csv(
...
measurements_table = filterByPcrDate(
    measurements_table, "measurement_date", firstPcrDate
)
...
def add_measurements(concept_id, column_name, df):
    selected_measurement = measurements_table[
        measurements_table.measurement_concept_id == concept_id
    ]
    ...
    for person_id in np.unique(selected_measurement.person_id.values):
        try:
            index = df[df.person_id == person_id].index[0]
            measurement_values = selected_measurement[
                selected_measurement.person_id == person_id
...
def add_conditions(df):
    ...
    for person_id in person_id_arr:
        ...
        unique_person_condition_list = np.unique(
            person_conditions_df.condition_concept_id.values
        )
...
person_table = pd.read_csv(
    "/data/person.csv",
    usecols=["person_id", "year_of_birth", "gender_concept_id", "race_concept_id"],
)
...
device_exposure = pd.read_csv(
    "/data/device_exposure.csv", usecols=["person_id", "device_concept_id"]
)
...
observation_table = pd.read_csv(
    "/data/observation.csv",
    usecols=["person_id", "observation_concept_id", "value_as_string"],
)
tobacco_table = observation_table[
    (observation_table.observation_concept_id == 4005823)
    & (observation_table.value_as_string == "Yes")
]
tmp_arr = np.zeros(person_table.shape[0])
...
training_columns = pd.read_csv(
    "/model/training_columns.csv", index_col=0
).columns.values
